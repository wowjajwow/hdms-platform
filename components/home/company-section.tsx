...
const tileRevealOrder = [8, 3, 6, 1, 10, 5, 0, 9, 4, 11, 2, 7];
    ...
    >
      ...
      >
        ...
        <div
          ...
          className="absolute inset-0 grid grid-cols-4 grid-rows-3 overflow-hidden"
        >
          {tileRevealOrder.map((tile, sequence) => {
            const column = tile % 4;
            ...
            return (
              <span
                key={tile}
                className="block bg-[url('/images/home/company-hdms-bg.png')] bg-no-repeat transition-[opacity,transform] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)]"
                style={{
                  gridColumn: column + 1,
                  gridRow: row + 1,
                  ...
                  transform: isVideoEnded ? "scale(1.015)" : "scale(0.72)",
                  transitionDelay: isVideoEnded ? `${sequence * 65}ms` : `${(11 - sequence) * 20}ms`,
                }}
              />
            );
          })}
